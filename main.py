...
def make_client_window(scores):
    table_data = []
    for i in range(len(scores["players"])):
        row = [scores["players"][i], scores["scores"][i], scores["previous_scores"][i]]
        table_data.append(row)
    client_layout = [[sg.Table(table_data, headings=["Players", "Scores", "Previous Scores"], key="score_table", hide_vertical_scroll=True,
                               num_rows=len(scores["players"]))]]
    return sg.Window("Jeopardy Score Client", client_layout, finalize=True)
...
def start_scorer_windows(players):
    ...
    scorer_layout.append([sg.Button("Update", key="update", bind_return_key=True), sg.Button("Start Client", key="client_start")])
    ...
    scores = {"players": players, "scores": ["0" for _ in range(len(players))], "previous_scores": ["0" for _ in range(len(players))]}
    while True:
        scorer_event, scorer_values = scorer_window.read()
        if scorer_event is None:
            return
        elif scorer_event == "client_start" and not client_window:
            scorer_window["client_start"].update(visible=False)
